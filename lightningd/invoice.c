					   ...
					   const jsmntok_t *params)
{
	...
	if (!param(cmd, buffer, params,
		   ...
		   p_opt("payment_preimage", param_tok, &hashtok),
	...
	if (hashtok) {
		/* Get payment hash from user. */
		if (!hex_decode(buffer + hashtok->start,
				hashtok->end - hashtok->start,
				&info->b11->payment_hash,
				sizeof(info->b11->payment_hash))) {
			return command_fail(cmd, JSONRPC2_INVALID_PARAMS,
					    "payment hash must be 64 hex digits");
		}
	} else {
		if (preimagetok) {
...
static const struct json_command invoice_command = {
	...
	"or optional {payment_hash} (default hash of preimage)"};
